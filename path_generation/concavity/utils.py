...
from shapely.geometry import Polygon, LineString, MultiPolygon, MultiLineString, LinearRing, MultiPoint
from shapely.ops import polygonize
import warnings
import rtree
from scipy.signal import savgol_filter

def gaussian_smooth_3d(geom, sigma=2, num_points_factor=1):
    ...
    x = gaussian_filter(x, sigma, mode='wrap')
    y = gaussian_filter(y, sigma, mode='wrap')
    coords[:, 0] = x
    coords[:, 1] = y
    return coords

def gaussian_smooth_3d1(geom, sigma=1, num_points_factor=1):
    coords = geom.exterior.coords
    coords = np.array(coords)
    x, y = coords[:, 0:2].T 
    xp = np.linspace(0, 1, coords.shape[0])
...
def gaussian_smooth(coords, sigma=2, num_points_factor=2):
    ...
    return x, y
def gaussian_smooth_1(coords, sigma=1, num_points_factor=2):
    coords = np.array(coords)
    x, y = coords.T #x, y = coords[:, 0:2].T #
    xp = np.linspace(0, 1, coords.shape[0])
    interp = np.linspace(0, 1,coords.shape[0] * num_points_factor)
    x = np.interp(interp, xp, x)
    y = np.interp(interp, xp, y)
    ...
    coords[:, 1] = y
    return x,y
def savgol_smooth(coords, sigma=0, num_points_factor=2):
    ...
    x = np.interp(interp, xp, x)
    y = np.interp(interp, xp, y)
    #SAVITZKY-GOLAY : use w points to compute order+1 polynomial coefficients
    stdev= np.std(x-y)
    alpha = ((len(coords))-1)/(2*stdev)
    odd_alpha = int(alpha/3)+1
    if odd_alpha %2 ==0:
        odd_alpha = odd_alpha+1
    window = odd_alpha
    x = savgol_filter(x,window,sigma, mode='wrap')
    y = savgol_filter(y,window,sigma, mode='wrap')
    return x, y

def smooth_geom(geom, sigma=0, num_points_factor=2):
    """
    ...
    if isinstance(geom, (Polygon, LineString)):
        x, y = savgol_smooth(geom.exterior.coords)
...
def gaussian_smooth_geom_1(geom, sigma, num_points_factor=2):
    """
    :param geom: a shapely LineString, Polygon, MultiLineString ot MultiPolygon
    :param sigma: standard deviation for Gaussian kernel
    :param num_points_factor: the number of points determine the density of vertices  - resolution
    ...
    if isinstance(geom, (Polygon, LineString)):
        x, y = gaussian_smooth_1(geom.exterior.coords)
        ...
        if type(geom) == Polygon:
            ...
            if len(list(geom.interiors)) > 0:
                ...
                for interior in list(geom.interiors):
                    x, y = gaussian_smooth_1(interior)
                    l.append(list(zip(x, y)))
                return Polygon([item for sublist in l for item in sublist])
            else:
