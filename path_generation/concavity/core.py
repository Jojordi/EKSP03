...
from .utils import make_ccw, ConcaveHull
...
def plot_vertices(geom, concave_vertices_df=None, convex_vertices_df = None, figsize = (10,10)):
    ...
    fig, ax = plt.subplots(figsize=figsize)
    legend_elements = []
    gpd.GeoSeries([geom]).plot(ax=ax, color='lightgrey', alpha=0.5, edgecolor='black')

    if concave_vertices_df is not None:
        concave_vertices_df.plot(ax=ax, color='navy')
        legend_elements += [Line2D([0], [0], markersize=8, color='w',
    ...
    if convex_vertices_df is not None:
        convex_vertices_df.plot(ax=ax, color='crimson')
        legend_elements += [Line2D([0], [0], markersize=8, color='w',
