...
theta= np.pi/(4) #Ángulo de la dirección del ráster
...
defectpcd = o3d.io.read_point_cloud("dirección aquí") #dirección del archivo PCD que contiene el defecto
...
axis.scale(10, (0, 0, 0)) #se escala el eje para archivos creados en CloudCompare
...
pcd1.translate((0,-35,-10)) #Traslación para ubicar la nube en el lugar deseado
pcd1.scale(5, (0, 0, 0)) #Escalamiento para nubes creadas en CloudCompare
...
n=0
defect=copy.deepcopy(pcd1)
defect.paint_uniform_color((0.3,0.3,0.3))
bbox = defect.get_axis_aligned_bounding_box() #Bounding box que encierra el defecto
bbox.translate((0,0,10)).scale(2, (0, 0, 0))
#defect=defect.crop(bbox) #Corte de capas según la bounding del defecto
defect.estimate_normals()
coat=fp.multioffset3d(defect,h,n,True,True,True,True,True,True)
coatvis=fp.addpoints(coat).crop(bbox)
o3d.visualization.draw_geometries([axis,defect])
o3d.visualization.draw_geometries([axis,coatvis])
coatvis.paint_uniform_color((1,0.7,0.2))
o3d.visualization.draw_geometries([axis,defect,coatvis])
...
fp.animate([finalpath],t=10) #Animación con matplotlib
